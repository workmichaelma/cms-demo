...
export const Field = {
  PermitArea: {
    vehicles: async (parent, args, contextValue, info) => {
      try {
        const Model = contextValue?.Model.permitArea
        ...
        if (parent && parent?._id) {
          ...
          if (vehicles && !isEmpty(vehicles)) {
            return vehicles.map((vehicle) => {
              return {
                doc_id: vehicle?.permit_areas?._id,
                effective_date: vehicle?.permit_areas?.effective_date,
                end_date: vehicle?.permit_areas?.end_date,
